...
class Fish:     
    ...
    def aggregate(self,direction,max_xy): 
        ...
        self.angle=(direction+np.random.uniform(-pi/8,pi/8)) 
        self.dx = speed * cos(self.angle)
        self.dy = speed * sin(self.angle)
        self.adjust_movement(max_xy)   
            ...
            for y in xrange(self.max_xy):
                if not(shark_presence[x,y]):
                    if (self.fish_school_density>len(fish_presence[x][y])>0):
                        max_density = 0
                        for sx in xrange(int(floor(x - self.fish_perceptual_range)),
                                         int(ceil(x+self.fish_perceptual_range+1))):
                            for sy in xrange(int(floor(y-self.fish_perceptual_range)),
                                             int(ceil(y+self.fish_perceptual_range+1))):                
                               if (sx!=x) and (sy!=y):
                        ...
                        if max_density == 0:
                            ...
                            direction=(acos(fabs(x-max_density_patch_coords[0])
                                        /(sqrt((x-max_density_patch_coords[0])**2
                                        +(y-max_density_patch_coords[1])**2))))
                            if (x-max_density_patch_coords[0]<0) and (y-max_density_patch_coords[1]>0): # prey to the lower right
                                direction-=pi/2                 # -90°
                            elif (x-max_density_patch_coords[0]>0) and (y-max_density_patch_coords[1]>0): # prey to the lower left 
                                direction+=pi                   # +180°
                            elif (x-max_density_patch_coords[0]>0) and (y-max_density_patch_coords[1]<0): # trap to the upper left
                                direction+=pi/2                                     
...
if __name__ == '__main__':                
    simulation=Simulation(max_time = 500000,
                          max_xy=30,                                                #Landscape boundaries
                          fish_max_speed=1,
                          shark_max_speed=1,
                          start_fish_pop=1500,
                          start_shark_pop=1,
